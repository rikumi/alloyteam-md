...
![noerror_fe](https://cloud.githubusercontent.com/assets/10385585/23819962/80913706-0649-11e7-9ffb-85ce40efad69.png)

# 错误信息分析・Script error

有了监控了后，就可以在收集平台上进行查看脚本错误量的日志统计。

![scripterrdata](https://cloud.githubusercontent.com/assets/10385585/23838826/702b42ee-07d4-11e7-9853-2c135550e00e.png)
...
![scripterror](https://cloud.githubusercontent.com/assets/10385585/23820174/bb73b3e0-064d-11e7-9bdb-5e382fb549d6.png)
...
![requestorigin](https://cloud.githubusercontent.com/assets/10385585/23839391/b15066b0-07d8-11e7-81eb-f2e0b63ec02b.png)

#### 2. 响应头中增加 Access-Control-Allow-Origin 来支持跨域资源共享。

![responseheader](https://cloud.githubusercontent.com/assets/10385585/23839417/eb2cde86-07d8-11e7-887f-4824f0dcafa8.png)

Access-Control-Allow-Origin: \* 表示通过该跨域请求，且该资源可以被任意站点跨站访问。而当该资源仅允许来自 [http://127.0.0.1:8066](http://127.0.0.1:8066/) 的跨站请求，其它站点都不能跨站访问时，将可以返回：
...
![varyorigin](https://cloud.githubusercontent.com/assets/10385585/23840619/aef5528c-07e1-11e7-8131-bc0b595ac2cc.png)

举例・不加 Vary 将存在错误命中缓存的问题

![crossoriginerror](https://cloud.githubusercontent.com/assets/10385585/23839839/aa8fb4ea-07db-11e7-8e9f-6258f6d610cd.png)

上图中，第一个请求（Origin: 127.0.0.1:8066）响应被浏览器缓存了，当第二个请求（Origin: 127.0.0.1:8888）发起，被错误命中了前一个请求的缓存，收到了 Access-Control-Allow-Origin:[http://127.0.0.1:8066](http://127.0.0.1:8066/) 的响应时，将导致资源加载失败。  
...
app.use(function* (next) {
    ...
    this.set("Access-Control-Allow-Origin", requestOrigin); // 设置 Vary: Origin
    this.vary("Origin");
    return yield next;
});
```

以上为本文所有内容，兄弟篇：(coming soon..)

[阅读更多文章 >>](https://github.com/joeyguo/blog/issues)  
<https://github.com/joeyguo/blog/issues>
