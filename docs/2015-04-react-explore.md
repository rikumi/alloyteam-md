...
var MovieListComponent = React.createClass({
    ...
    getInitialState: function () {
        return {
            loading: true,
            title: "我喜欢的电影", // 注意这里将 外部传入的数据赋值给了 this.state
            movies: [],
        };
    }, // 使用 render 方法自动渲染 DOM
    render: function () {
        // this.state 用于存储数据
        var title = this.state.title; // this.props 用于从组件外部传入数据
...
-   render () 返回的是一系列嵌套的组件 this.props 获取父组件传递给子组件的数据 this.setState ({data: data}); 用于动态更新状态，设置数据（设置后 UI 会自动刷新）
