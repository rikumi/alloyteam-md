...
class MainThreadController {
    ...
    onmessage(e) {
        ...
        if (!this.actionHandlerMap[id]) return; // 执行相应的 Promise resolve
...
mainThreadController
    ...
    .then((response) =>
        console.log("message received from worker: ", response.msg)
    );
```
    ...
    async onmessage(e) {
        ...
        switch (actionType) {
            case "print":
                ...
                self.postMessage({
                    id,
                    response: { msg: "msg has been print." },
                });
                break;
            case "asyncCalc": // 构造一个异步处理情形
                const result = await new Promise((resolve) =>
                    setTimeout(() => resolve(payload.params * 2), 1000)
                );
                self.postMessage({
                    id,
                    response: { msg: `the caculated answer is ${result}.` },
                });
                break;
            default:
                break;
