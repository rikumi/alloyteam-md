...
function compose(middleware) {
    ...
    return function (context, next) {
        ...
        let index = -1; // 在这里开始处理处理第一个中间件
        return dispatch(0); // dispatch 是核心的方法，通过不断地调用 dispatch 来处理所有的中间件
        function dispatch(i) {
            if (i <= index) {
            ...
            index = i; // 获取中间件函数
            let handler = middleware[i]; // 处理完最后一个中间件，返回 Proimse.resolve
            ...
            try {
                ...
                return Promise.resolve(
                    handler(context, dispatch.bind(null, i + 1))
                );
            } catch (err) {
