...
![deploy keys in github](https://user-gold-cdn.xitu.io/2017/8/30/11adc00f353c3dbb399da1e64dbc9d7c)
...
![travis encrypt-file result](https://user-gold-cdn.xitu.io/2017/8/30/9043b9c5c4ca69a097211b45b694912c)
...
![bitbucket](https://user-gold-cdn.xitu.io/2017/8/30/e0a8f61e5c4c9e2b0cfba194b965274d)
...
![1st part Github Webhook](https://user-gold-cdn.xitu.io/2017/8/30/9a15691d28d7518740c29f65836a1065)

![2nd part of Github Webhook](https://user-gold-cdn.xitu.io/2017/8/30/0b3ed99c8c2bcbdcd244996371d5bd66)  

Bitbucket Webhook

以下是 `Bitbucket` 的 `Webhook` 设置截图。

![bitbucket](https://user-gold-cdn.xitu.io/2017/8/30/d450260c3b53af5ba889e19771c292f1)  

## 部署服务

腾讯云 COS

COS，即对象存储（Cloud Object Storage，是面向企业和个人开发者提供的高可用，高稳定，强安全的云端存储服务。您可以将任意数量和形式的非结构化数据放入 COS，并在其中实现数据的管理和处理。COS 支持标准的 Restful API 接口，是非常适合作为静态文件部署存储服务。

首先，创建一个 Bucket，没有备案的域名请选择海外的节点，同时选择 CDN 加速（后面需要用）：

![txqun1](https://user-gold-cdn.xitu.io/2017/8/30/f0db570f99ae6143af6288018d472cb3)

创建后，会进入 `Vue Bucket` 的文件列表页面，目前是空的。到 **基础配置**，中开启\*\*_静态网站_配置。

![txqun2](https://user-gold-cdn.xitu.io/2017/8/30/fa28d1d706ece51be959f1bbe2146abf)

然后进入**域名管理**，添加域名。

![txqun3](https://user-gold-cdn.xitu.io/2017/8/30/f31549755c4026c5f6a4abfd9e6c8657)

然后到你的域名管理提供商，配置一下 `CNAME` 解析。直接用上图的域名和 `CNAME` 值。

![txqun4](https://user-gold-cdn.xitu.io/2017/8/30/62e6c730353764c0351409f74c4d474f)

如果需要开启 `HTTP2`，还可以去申请 `HTTPS` 证书，腾讯云可以申请免费的 `HTTPS` 证书。  
![txqun6](https://user-gold-cdn.xitu.io/2017/8/30/79ee643a812c0602f804ab1da22d63cd)

腾讯云 CDN

等域名 CDN 加速部署好后，可以点击 COS **域名配置** 中，具体域名的 **高级配置**，进入 **CDN** 管理界面。

进入 **缓存配置**，添加一些缓存规则，如 `js, css, png, jpg` 等带有 `md5` 的资源可以长久缓存，而 `html` 等资源则不宜缓存。

![txqun7](https://user-gold-cdn.xitu.io/2017/8/30/0d98fa9eccb4ee8c689080baabaa7f97)
