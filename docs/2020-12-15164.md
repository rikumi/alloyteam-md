...
date: 2020-12-31
author: TAT.vorshen
source_link: http://www.alloyteam.com/2020/12/15164/
---
...
### [HashTable 原理](http://www.alloyteam.com/2020/12/15164/#hashtable-原理-1)

### [HashTable 问题与解决](http://www.alloyteam.com/2020/12/15164/#hashtable-问题与解决-1)

### [v8 中的 HashTable](http://www.alloyteam.com/2020/12/15164/#v8-中的-hashtable-1)

### [redis 中的 HashTable](http://www.alloyteam.com/2020/12/15164/#redis-中的-hashtable-1)
...
![](http://115.159.126.29/blog/hash-table/assets/1.png)
...
_v8 下 name.h 的注释，可以看到 **All names store a hash value.**。_

这样我们第一个问题基本解决清楚了，更深的细节比如 _gcc 提供的通用 hash 能力_、_如何尽可能避免 hash 碰撞等_，可以自行了解。

Q: **冲突了怎么办**  
从之前的流程可以看到，存在两个阶段可以冲突  
![冲突的两个阶段](http://115.159.126.29/blog/hash-table/assets/2.png)
...
![开放寻址法插入元素](http://115.159.126.29/blog/hash-table/assets/3.png)
...
![开放寻址法查找元素](http://115.159.126.29/blog/hash-table/assets/4.png)
...
1.  **数组长度不够了怎么办**  
    _当装载因子变成 1 的时候，无论怎么寻址，对应位非空，空间已经不够用了。那时候我们就得重新申请更长的数组长度，这个过程一般称为 rehash_。
2.  **一直查找不到的情况下，会不会死循环**  
    _插入类型，必须要保证空间足够_  
    _查找类型不会死循环，当寻找到空位置或者原点的时候，就认为不在表中_。
3.  **如何确保不会取错**  
    _因为 entry 甚至 hashKey 都可能存在冲突，所以切记不能只存一个 value。key (原始 key，因为 hashKey 可能已经冲突了) 也需要存，比对的时候需要比对 key_。
4.  **删除导致的空，会影响寻址的地址，这里用个图举例**

首先插入 C，但是发现 entry=1 存在，所以真实放到了 entry=2  
![插入 C](http://115.159.126.29/blog/hash-table/assets/3-1.png)

删除了 B，如果就直接将 entry=1 清空  
![删除 B](http://115.159.126.29/blog/hash-table/assets/3-2.png)

此时再查找 C，hash 之后发现 entry=1 为空，那就认为 C 不在表中，打出 GG  
![查找 C](http://115.159.126.29/blog/hash-table/assets/3-3.png)
...
先用图来讲解下链表法插入元素  
![链表法查找元素](http://115.159.126.29/blog/hash-table/assets/5.png)  
...
![yori 内存](http://115.159.126.29/blog/hash-table/assets/6.png)  
![yussica 内存](http://115.159.126.29/blog/hash-table/assets/7.png)
...
内存结构变成了这样  
![yori 内存](http://115.159.126.29/blog/hash-table/assets/8.png)  
![yussica 内存](http://115.159.126.29/blog/hash-table/assets/9.png)  
可以看到 yori 数组还是很大，基本上还是满足刚刚的规律的。  
但是！yussica 数组占用空间变得很小了，不过对于开发使用起来并没有感觉到什么差异，这是为什么呢？

因为当数组长度超过 32 _1024_ 1024 时，JSArray 的内部实现，会由 FastElement 模式（FixedArray 实现），变成 SlowElement 模式（HashTable 实现）  
...
_这里标注出了内部的方法名，方便感兴趣看源码的同学可以自行查阅_
...
![缩减版 UML](http://115.159.126.29/blog/hash-table/assets/10.png)
...
![HashTable 内存分布](http://115.159.126.29/blog/hash-table/assets/11.png)
...
3.  计算 entry，结果分别是 3、0、1  
    **这里注意了！在 Capacity 为 4 的情况下，直接 Mask 与运算得到的结果是如下的**  
    ![运算结果](http://115.159.126.29/blog/hash-table/assets/12.png)
...
_// EnsureCapacity will guarantee the hash table is never full._
...
我们不卖关子了，ES6 中的 Set 和 Map 就是用链表法实现的 HashTable，内部类是 OrderedHashTable。或许你已经对 _Ordered_ 关键字开始疑惑了，不过不用担心，让我们一步一步分析。
...
结果如图  
![排序结果](http://115.159.126.29/blog/hash-table/assets/13.png)  
可以看到是按字典序排序的

那 js 中的 Map 有序么？在控制台执行发现很神奇的现象是有序的，但是和 C++ 中的 map 又不太一样，它不是按照字典序，而是按照插入的顺序  
![运算结果](http://115.159.126.29/blog/hash-table/assets/14.png)

是不是很神奇，v8 在这里底层用的是 OrderedHashTable，这里先给一下 OrderedHashTable 的内存分布图  
![OrderedHashTable 内存分布图](http://115.159.126.29/blog/hash-table/assets/15.png)  
...
_先给一个噩耗，v8 目前这里对 Map、Set 的接口相关有点乱。感兴趣看源码的同学可以直接看 js-collection.h，看数据结构清晰很多。不建议直接跟执行流程，如果硬要跟，看 builtins-collections-gen.cc_
...
![操作步骤](http://115.159.126.29/blog/hash-table/assets/16.png)
...
![](http://115.159.126.29/blog/hash-table/assets/17.png)  
![](http://115.159.126.29/blog/hash-table/assets/18.png)  
![](http://115.159.126.29/blog/hash-table/assets/19.png)  
_之所以添加到表头，也是 redis 认为最近添加的元素，被访问到的几率大一些_

a,b,c,d 的设置讲清楚了，再看下面 e,f,g,h 的设置，这里涉及到**增量 rehash** 了，稍微复杂一些。  
我们先继续看图片流程，理解了流程，看代码就清晰了。  
![](http://115.159.126.29/blog/hash-table/assets/20.png)  
![](http://115.159.126.29/blog/hash-table/assets/21.png)  
![](http://115.159.126.29/blog/hash-table/assets/22.png)  
![](http://115.159.126.29/blog/hash-table/assets/23.png)  
![](http://115.159.126.29/blog/hash-table/assets/24.png)  
![](http://115.159.126.29/blog/hash-table/assets/25.png)  
![](http://115.159.126.29/blog/hash-table/assets/26.png)  
![](http://115.159.126.29/blog/hash-table/assets/27.png)  
![](http://115.159.126.29/blog/hash-table/assets/28.png)  
![](http://115.159.126.29/blog/hash-table/assets/29.png)
