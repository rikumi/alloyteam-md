...
![go 协程执行斐波拉切数列的耗时](https://raw.githubusercontent.com/vorshen/blog/master/thread/assets/1.png)
...
![递归改成迭代的耗时](https://raw.githubusercontent.com/vorshen/blog/master/thread/assets/2.png)
...
_单线程执行两个耗时 5s 的操作结果_

![单线程执行两个耗时 5s 的操作结果](https://raw.githubusercontent.com/vorshen/blog/master/thread/assets/time_cost_1.png)
...
![多线程 + 限制单核执行](https://raw.githubusercontent.com/vorshen/blog/master/thread/assets/time_cost_2_single_core.png)

多核情况就不一样了，下面我们放开 cpu 的限制，可以明显看到，real time 直接少了一半，但是 user time 不变。

_多线程无限制执行_

![多线程无限制执行](https://raw.githubusercontent.com/vorshen/blog/master/thread/assets/time_cost_2_mutil_core.png)
...
![执行情况 A](https://raw.githubusercontent.com/vorshen/blog/master/thread/assets/conflict_1.png)

_执行情况 B_

![执行情况 B](https://raw.githubusercontent.com/vorshen/blog/master/thread/assets/conflict_2.png)

_执行情况 C_

![执行情况 C](https://raw.githubusercontent.com/vorshen/blog/master/thread/assets/conflict_3.png)
...
![不同的顺序会带来不同的结果](https://raw.githubusercontent.com/vorshen/blog/master/thread/assets/conflict_a.png)

所以当遇到写冲突的时候，就会出现覆盖，然后每次增加之前又取最新的值，所以就会出现执行的现象

针对 B，汇编的代码是这样 _（-O1 优化）_
...
![单一堆栈且清晰](https://raw.githubusercontent.com/vorshen/blog/master/thread/assets/debugger1.png)

基本上单线程调试的复杂度，是随着代码复杂度，常数级增加（一般不会超过线性级）。

但是涉及到多线程，这里复杂度就会明显上升：

![多堆栈且复杂](https://raw.githubusercontent.com/vorshen/blog/master/thread/assets/debugger2.png)
...
![这里启动了两个进程](https://raw.githubusercontent.com/vorshen/blog/master/thread/assets/nginx1.png)
