...
const findOneProjectByName = (name) =>
    new Promise((resolve, reject) => {
...
componentDidMount = () => {
    ...
    socket.on("connect", () => {
        socket.on("component", (data) => {
            if (
                data.project === this.state.component.project &&
                data.component === this.state.component.name
            ) {
                console.log(
                    "component changed: ",
                    data.project,
                ...
                this.showSnackbar("本地组件已更新, 自动刷新"); // 重新向服务器拉取最新编译好的组件, 进行更新
                this.getComponent(data.project, data.component);
            }
        });
    });
